<script>
  ...
  $: xScaleTimeInv = scaleLinear().domain([0, width]).range([0, tmax])
  ...
  var drag_intervention = function () {
    ...
    var dragged = function (d) {
      if (actionMarkerData.isConfigurable()) {
        const draggedX =
          InterventionTimeStart + xScaleTimeInv(event.x - dragstarty)
        const minX = 0
        const maxX = tmax - 1
        // if (draggedX < minX-3) {
        ...
        //     flashMessage = ''
        // }
        actionMarkerData[AM_DAY] = Math.round(
...
<div
  style="position: absolute; width:{width +
    15}px; height: {height}px; position: absolute; top:100px; left:10px; pointer-events: none; z-index: {zIndex};"
  title={actionMarkerData[AM_NAME]}
>
  ...
  <div
    ...
    style="pointer-events: all;
      ...
      1}px;
                                ...
                                pointer-events: all;
                                cursor:{actionMarkerData.isConfigurable()
      ? 'col-resize'
      : 'not-allowed'};
                                height:{height + 19}px;"
  >
    ...
    >
      {#if actionMarkerData.isConfigurable()}
        <div
          style="top:-23px; left:-7px; position: absolute; text-align: right; width: 20px; height:20px; opacity: 0.3"
        >
          <svg width="20" height="20">
            <g transform="rotate(90)">
              <g transform="translate(0,-20)">
                <path
                  d="M2 11h16v2H2zm0-4h16v2H2zm8 11l3-3H7l3 3zm0-16L7 5h6l-3-3z"
                />
              </g>
            </g>
          </svg>
      ...
      <div
        class="caption"
        style="pointer-events: none; position: absolute; left:1px; top:0px; height: 60px; min-width:220px; border-left: 3px solid #777; padding: 5px 7px 7px 7px; "
      >
        <div class="paneltext" style="height:20px; text-align: right">
          ...
          {#if actionMarkerData.isConfigurable()}
            <div class="paneldesc unselectable">
              <span
                style="color: {InterventionAmt < 1 ? '#4daf4a' : '#f0027f'}"
              >
...
{#if actionMarkerData.isConfigurable()}
  <div
    style="position: absolute; width:{width +
      15}px; height: {height}px; position: absolute; top:140px; left:20px; pointer-events: none; z-index: {zIndex +
      1};"
  >
    <div
    ...
    >
      <div
        ...
        style="margin-top: 0px; height: 20px; font-style: italic;"
      ...
      >
        ...
        <span style="position: absolute; left: 20px;"
          >{actionMarkerData[AM_EXPANDED] ? 'Hide config' : 'Configure'}</span
        >
      </div>
      ...
      {#if actionMarkerData[AM_EXPANDED]}
        ...
        <div style="background-color: #FFF;">
          <div class="caption paneldesc unselectable" style="margin-top: 6px;">
            Name
          </div>
          ...
          <input
            ...
            style="width: 100%;"
            type="range"
            bind:value={actionMarkerData[AM_EFFECT]}
            min="-2"
            max="1"
            step="0.01"
            on:mousedown={lock_yaxis}
          />
          <div
            class="caption paneldesc unselectable"
