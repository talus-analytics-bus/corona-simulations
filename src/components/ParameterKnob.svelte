<script>
  ...
  function f(chapterTitle, chapterText) {
    ...
    return `<p><b>${stylizeExpressions(
      chapterTitle
    )}</b></p><p>${stylizeExpressions(chapterText)}</p>`
  }
  ...
  function displayPopup() {
    popupHTML = `${f(p.description, p.longformDescription)}
                     ${f(
                       ...
                       p.longformDoNotConfuseWith
                     )}
                     ${f('Effects', p.longformEffects)}
                     ${f(
                       'Justification for default value',
                       p.longformDefaultValueJustification
                     )}`
  }
      ...
      <div on:click={() => (value = p.defaultValue)} title="Estimate from data">
        ...
        {#if value === p.defaultValue}
          <Icon
            ...
            scale="0.8"
            class="flaskIcon"
            style="color: #003399; position: absolute; right: 0px; top: 20px;"
          />
        {:else}
          <Icon
            data={flask}
            scale="0.8"
            class="flaskIcon"
            style="color: #CCC; position: absolute; right: 0px; top: 20px;"
          />
        {/if}
      </div>
    {:else}
      <div
        on:click={() => (value = p.defaultValue)}
        title="Default value"
        style="font-size: 16px;"
      >
        <button
          class="clickableIcons unselectable"
          style="color: {value === p.defaultValue
            ? '#777'
            : '#CCC'};
            position: absolute;
            right: 0px;
